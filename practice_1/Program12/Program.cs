...
namespace Program12
{
            ...
            while (_clients.Count > 0)
            {
                ...
                if(int.TryParse(userInput, out int computerNumber))
                {
                    ...
                    if (computerNumber >= 0 && computerNumber < _computers.Count)
                    {
                        ...
                        {
                            Console.WriteLine("Вы попытались посадить клиента, за компьютер, который уже занят. Он разозлился и ушёл.");
                        }
                        else
                        {
    ...
    class Computer
    {
        ...
        private int _minuteRamining;

        public bool IsTaken
        {
            get
            {
                return _minuteRamining > 0;
            }
        }
        ...
        {
            _client = client;
            _minuteRamining = _client.DesiredMinutes;
        }
        ...
        {
            if (IsTaken)
                Console.WriteLine($"Компьютер занят, осталось минут: {_minuteRamining}");
            else
